...
impl tcp {
    pub fn action(&mut self, buf: &[u8], nbytes: usize, nic: &mut tun_tap::Iface) { // is it a good choice to leave nic here?
                        ...
                        match self.flow_table.entry(q){
                            ...
                            Entry::Vacant(e)=>{
                                ...
                                if self.listening.contains(&q.dst.1){
                                    if let Some(new_f)= flow::flow::three_way_handshake(nic,iph,tcph).unwrap(){
                                        e.insert(new_f);
                                    }else{
    ...
    pub fn control(&mut self,message:control_message){
        match message {
            ...
            control_message::Read=>{
                unimplemented!()
            }
            control_message::Write=>{
                unimplemented!()
            }
        }
    }
